# Example Usage: OpenWRT Wireless Configuration System

This document provides practical examples of using the new router configuration file system.

## Quick Start Example

Let's say you have 3 routers and want to set up wireless networks across all of them.

### Step 1: Set Up Your SSID Configurations

Your shared SSID configs are already in `wireless-configs/`:
- `ssid_main.conf` - Your main home network
- `ssid_guest.conf` - Guest network
- `ssid_iot.conf` - IoT devices network
- `common-overrides.conf` - Hardware-specific settings for all routers

### Step 2: Create Router Configuration Files

Create individual config files for each router in the `routers/` directory:
...
Now you can deploy to your routers using simple commands:

```bash
# Deploy to single router (test first)
./deploy-wireless.sh -d routers/living-room.conf

# Deploy to single router (actual deployment)
./deploy-wireless.sh routers/living-room.conf

# Deploy to all routers
./deploy-wireless.sh routers/*.conf

# Deploy to specific routers
./deploy-wireless.sh routers/living-room.conf routers/bedroom.conf
```
...
SSID_OVERRIDE_guest_disabled="1"
SSID_OVERRIDE_iot_bands="2g"  # 2.4GHz penetrates better
```

2. **Test and deploy:**
```bash
./deploy-wireless.sh -d routers/basement.conf  # Test first
./deploy-wireless.sh routers/basement.conf     # Deploy
```

### Scenario 2: Updating Multiple Routers
...
### Scenario 2b: Updating Hardware-Specific Settings
...
### Scenario 3: Router-Specific Customization
...
# IoT network for office devices (printers, etc.)
SSID_OVERRIDE_iot_extra="isolate=1 max_num_sta=30"
```

2. **Deploy just the office router:**
```bash
./deploy-wireless.sh routers/office.conf
```

### Scenario 5: SSH Key Authentication
...
```bash
./deploy-wireless.sh routers/secure-router.conf
```
...
Different router models need different settings. Most hardware-specific optimizations should go in `wireless-configs/common-overrides.conf` to apply to all routers:

**Global hardware optimizations** (`wireless-configs/common-overrides.conf`):
```bash
#!/bin/sh
...
```bash
./deploy-wireless.sh -v routers/problem-router.conf
```
...
# Test all routers
./deploy-wireless.sh -d routers/*.conf
...
4. **Set country code only if needed:** Country code has no default - only set `COUNTRY_CODE` if your deployment requires it
5. **Document changes:** Add comments explaining why you made specific overrides
6. **Test incrementally:** Deploy to one router first, then expand
7. **Keep backups:** Router configs are small, keep backups of working configurations
8. **Use descriptive names:** Router names should match their physical location/purpose
9. **Consistent naming:** Use consistent naming schemes for easy management
...
```bash
# Disable main network, only guest
...
This approach gives you complete control over each router while maintaining shared network definitions and simple deployment workflows.
